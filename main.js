    ...
    fast.onclick = function(){
      ...
      if (fast.classList.contains('buttons__item--checked')) {
        for (let i = 0; i <= tickets.length - 1; i++) {
          ...
          for (let b = i + 1; b <= tickets.length - 1; b++) {
            if (tickets[b].segments[0].duration < minValue.segments[0].duration && tickets[b].segments[1].duration < minValue.segments[1].duration && tickets[b].segments[0].duration < minValue.segments[1].duration && tickets[b].segments[1].duration < minValue.segments[0].duration) {
              minValue = tickets[b];
              let swap = tickets[i];
              tickets[i] = minValue;
              tickets[b] = swap;
